...
void inverse_matrix(int *, int*, int);      /* return the number of neighbors of a node */
...
int main(int argc, char *argv[])
{
    ...
    int *graph_inverse = NULL; /* inverse graph */
    char *nbnodes = argv[2];
    ...
    int *sendcountsgraph = NULL; /* array describing how many elements to send to each process */
    int *displs = NULL;            /* array describing the displacements where each segment begins */
    int *displsgraph = NULL;       /* array describing the displacements where each segment begins */
    int rem = nb_nodes % numtasks; /* elements remaining after division among processes */
    int remgraph = nb_nodes % numtasks; /* elements remaining after division among processes */
    int sum = 0;                   /* sum of sendcounts */
    int sumgraph = 0;              /* sum of sendcountsgraph */
    ...
    sendcountsgraph = calloc(numtasks, sizeof(int));
    displsgraph = calloc(numtasks, sizeof(int));
    ...
    if (rank == 0)
    {
        ...
        graph_inverse = calloc(nb_nodes * nb_nodes, sizeof(int));
        ...
        for(int i=0; i<nb_nodes; i++)
        {
            ...
            {
                if(graph[i*nb_nodes+j])
                {
                    number_of_neighbors[i]++;
                }
            }
        }
        inverse_matrix(graph, graph_inverse, nb_nodes);
        /* initialize pagerank */
        for (int i = 0; i < nb_nodes; i++)
        {
        ...
        for (int i = 0; i < numtasks; i++)
        {
            ...
            sendcountsgraph[i] = sendcounts[i]*nb_nodes;
            displs[i] = sum;
    ...
    MPI_Bcast(sendcountsgraph, numtasks, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Bcast(displsgraph, numtasks, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    /* scatter graph */
    MPI_Scatterv(graph_inverse, sendcountsgraph, displsgraph, MPI_INT, buff_graph, sendcountsgraph[rank], MPI_INT, 0, MPI_COMM_WORLD);
    ...
    for (int k = 0; k <= nb_iterations; k++)
    {
        ...
        {   
            FILE *fptr;
            fptr = fopen("result_pagerank_mpi_litememory.txt", "a");
            printf("iteration %d\n", k);
    ...
    if (rank == 0)
        {free(pagerank);pagerank = NULL;free(graph);graph = NULL;}
    free(pagerank_old);
    pagerank_old = NULL;
    free(sendcounts);
    sendcounts = NULL;
    ...
    free(buff_graph);
    buff_graph = NULL;
    free(sendcountsgraph);
    sendcountsgraph = NULL;
    free(displsgraph);
    displsgraph = NULL;
...
void inverse_matrix(int* graph, int* graph_inverse,int nb_nodes)
{
    for( int i=0;i<nb_nodes;i++)
    {
        for(int j=0;j<nb_nodes;j++)
        {
            graph_inverse[i*nb_nodes +j ]= graph[j*nb_nodes +i];
